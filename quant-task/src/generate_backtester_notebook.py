...
def _source_lines(text: str) -> List[str]:
    # Jupyter accepts either a single string or a list of strings.
    # We use a list for stable diffs and clear line boundaries.
    lines = text.splitlines(keepends=True)
    if not lines:
        return [""]
    # Ensure the last line ends with a newline for consistent rendering.
...
def create_notebook() -> None:
    ...
    cells.append(
        _md(
            "Note: This notebook uses local CSVs under `dataset/cleaned/` via `load_cleaned_assets()`. "
    ...
    cells.append(_md("### 3.1 Micro model (standalone): SMA crossover -> alpha"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 3.2 Allocator (standalone): Top-K long/short"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 3.3 Macro model (standalone): risk-on / risk-off"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 3.4 Regular model (standalone): direct weights"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 4.1 Regular model only"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 4.2 Micro model + allocator"))
    cells.append(
        _code(
    ...
    cells.append(_md("### 4.3 Micro + allocator + macro (combined)"))
    cells.append(
        _code(
    ...
    out_path = os.path.join("notebooks", "Backtester_Documented.ipynb")
